			...
			success:function(json){
				...
				for(var i=0; i<members.length; i++){
					...
					var p_access = members[i].p_access;
						...
						var td4 = $("<td>").html("<input type='button' value='삭제' class='delMem  a"+i+" btn btn-info'/> <input type='hidden' value='"+jyp_no+"'/>");
						//데이터 삽입하기 
			...
			success:function(json){
				...
				for(var i=0; i<members.length; i++){
					...
					console.log(p_access);
						...
						var td2 = $("<td>").html(dept);
						var td3 = $("<td>").html(position);		
						var td4 = $("<td>").html("<input type='button' value='추가' class='addMem  a"+i+" btn btn-info'/> <input type='hidden' value='"+jyp_email+"' />");
						//데이터 삽입하기 
						tr.append(td1).append(td2).append(td3).append(td4);
						$(".db2 tbody").append(tr);			
				...
				//	alert('추가 클릭시 jyp_email : ' + $(this).next().val());
					 $.ajax({
						 url:"${pageContext.request.contextPath}/p_member.pro",   
							type:"get", 
							dataType :"text",   
							data:{"p_no": p_no , "p_access":"1", "jyp_email":$(this).next().val()},
							success:function(text){
								console.log(text);
								var result =text; 
							//	alert(result);
