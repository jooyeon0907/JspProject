...
<script>
$(function(){
	$("#p_search1111").on( "click", function(){
		...
		$.ajax({
			...
			success:function(json){
				...
				for(var i=0; i<members.length; i++){
					...
					console.log("1-1 : " + p_access);
						//2. 동적 html
						var tr = $("<tr>");
						var td1 = $("<td>").html(jyp_name);
						var td2 = $("<td>").html(dept);
						var td3 = $("<td>").html(position);	
				//		var td4 = $("<td>").html("<a href='${pageContext.request.contextPath}/p_delete.pro?jyp_no="+jyp_no+"' class='btn btn-info' >삭제</a>");
						var td4 = $("<td>").html("<input type='button' value='삭제' class='delMem  a"+i+" btn btn-info'/>");
						var td5 = $("<td>").html("<input type='hidden' value='"+jyp_email+"' id='jyp_email'/><input type='hidden' value='"+jyp_no+"' id='jyp_no'/>");
						//데이터 삽입하기 
							...
							//	alert(result);
								if(result.trim()=="OK"){
									//	console.log('예');
										...
										// 2. 
										btn.attr("value","추가");
										btn.attr("class","addMem  a"+i+" btn btn-info");
										$(".db2 tbody").append(tr);	
			...
			success:function(json){
				...
				for(var i=0; i<members.length; i++){
					...
					console.log(p_access);
						...
						var td2 = $("<td>").html(dept);
						var td3 = $("<td>").html(position);		
						var td4 = $("<td>").html("<input type='button' value='추가' class='addMem  a"+i+" btn btn-info'/>");
						var td5 = $("<td>").html("<input type='hidden' value='"+jyp_email+"' id='jyp_email'/><input type='hidden' value='"+jyp_no+"' id='jyp_no'/>");
						//데이터 삽입하기 
						tr.append(td1).append(td2).append(td3).append(td4).append(td5);
						$(".db2 tbody").append(tr);			
							...
							//	alert(result);
								if(result.trim()=="성공"){
									...
									//	alert('프로젝트멤버추가완료');//
										...
										// 2. 
										btn.attr("value","삭제");
										btn.attr("class","delMem  a"+i+" btn btn-info ");
										$(".db1 tbody").append(tr);	
								//		location.href='${pageContext.request.contextPath}/pro_list.pro';
