...
public class WS_MemberSearch implements MAction {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int ws_no = (int)session.getAttribute("ws_no");  System.out.println("WS_MemberSearch에서 ws_no : " + ws_no);// 1;
		int p_no = Integer.parseInt(request.getParameter("p_no"));  System.out.println("WS_MemberSearch에서 p_no : " + p_no);//22; 
		
		dto_ws.setWs_no(ws_no);
		dto_p.setP_no(p_no);
		
		out.println(dao.ws_memSearch(dto_p, dto_ws)); //검색된 회원 정보 보내기 
