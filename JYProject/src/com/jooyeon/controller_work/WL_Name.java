...
public class WL_Name implements MAction {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int jyp_no_ws = dto_ws.getJyp_no(); System.out.println("WL_Name에서 jyp_no_ws: " + jyp_no_ws); 
		...
		if(jyp_no_ws==jyp_no || p_access==1 || jypno_Check == jyp_no){
			...
			String wl_name = request.getParameter("wl_name_edit");
			dto_wl.setWl_name(wl_name);
			
			int result = dao_wl.wl_edit_name(dto_wl);
			if(result>0){ out.println("<script>alert('업무list 이름수정 완료'); "
					+ " location.href='"+request.getContextPath()+"/list.work?p_no="+p_no+"';</script>"); }
			else {  out.println("<script>alert('업무list 이름수정 ERROR'); history.go(-1);</script>");  }
		}else {
