...
public class WS_InviRefuse implements MAction {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = dao.ws_refuse(dto); // 수락상태 y로 바꾸기 
		if(result>0){out.println("<script>alert('초대를 거절하였습니다.');"
				+ "location.href='"+request.getContextPath()+"/login2.members';</script>");}
		else{out.println("<script>alert('초대거절  ERROR'); history.go(-1);</script>");}
